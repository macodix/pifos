...
| 0.10 | 2026-07-03 | macodix | Kap. 9: Kommando-Schnittstelle verkürzt — Zieldatei als positionsgebundenes Argument statt `--output` (bei `--edit` optional, sonst Bearbeiten an Ort und Stelle ohne `--overwrite`-Zwang); `--format`/`--input-format` optional mit Ableitung aus der Dateiendung. |
