...
**Sicherung:** Die Sicherungsdatei heißt `<name>.bak` und liegt im Verzeichnis der Zieldatei oder unter dem einstellbaren `backup_location`. Sie wird exklusiv angelegt (`O_EXCL`, `O_NOFOLLOW`) und übernimmt die Rechte des Originals, ohne sie auszuweiten. Existiert die Sicherungsdatei bereits — etwa aus einer früheren Änderung derselben Datei —, schlägt die Sicherung mit `ActionError` fehl; das aufrufende Modul vermeidet solche Kollisionen, etwa über ein eigenes `backup_location` je Lauf.
